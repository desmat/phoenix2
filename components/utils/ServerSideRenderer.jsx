  ...
  match({routes, location}, (error, redirectLocation, renderProps) => {
    ...
    if (renderProps.components.length > 0 && renderProps.components[0] && (typeof renderProps.components[0] !== 'undefined')) {
      ...
      if (datae.length == 0) { 
        ...
        //if path ends with integer we'll expect an id
        var reqPathSplit = req.path.split('/');
        var pathId = parseInt(reqPathSplit[reqPathSplit.length - 1]);

        //TODO: fetch data in parallel

        _.each(datae, function(data, i) {

          //check if data contains keyword :id
          if (!isNaN(pathId) && data.indexOf(':id') > -1) {
